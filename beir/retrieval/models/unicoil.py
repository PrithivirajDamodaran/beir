...
class UniCOIL:
    ...
    def encode_query(self, query: str, batch_size: int = 16, **kwargs):
        embedding = np.zeros(self.bert_input_emb, dtype=np.float32)
    ...
    def encode(
        ...
        col = np.zeros(non_zero_tokens, dtype=np.int32)
        row = np.zeros(non_zero_tokens, dtype=np.int32)
        values = np.zeros(non_zero_tokens, dtype=np.float32)
        sparse_idx = 0    
        ...
        return csr_matrix((values, (col, row)), shape=(len(sentences), self.bert_input_emb), dtype=np.float32)
...
#         embeddings = np.zeros((len(sentences), self.sparse_vector_dim), dtype=np.float32)
...
#         return embeddings
#         # return csr_matrix((values, (row, col)), shape=(len(sentences), self.sparse_vector_dim), dtype=np.float32).toarray()
